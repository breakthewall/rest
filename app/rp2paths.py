...
def read_args_from_request(request):
    return
    # file = json.dumps(request.files['rp2_pathways'].read())
...
#    if logger==None:
    ...
    with tempfile.TemporaryDirectory() as tmpOutputFolder:
        ...
        with open(tmpOutputFolder+'/tmp_rp2_pathways.csv', 'wb') as outfi:
            outfi.write(json.loads(file))

        rp2paths_command = 'python3 /usr/src/app/rp2paths/src/RP2paths.py all '+str(rp2_pathways)+' --outdir '+str(tmpOutputFolder)+' --timeout '+str(int(args['timeout']*60.0+10.0))
        ...
        try:
            ...
            try:
                ...
                result_bytes = json.dumps(tar_cz_relative(tmpOutputFolder))
#                return b'OK'
